      ...
      <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
            <div className="space-y-2">
              ...
              <input
                id="name"
                type="text"
                {...register('name', { required: 'Vul je naam in alsjeblieft.' })}
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                {...register('message', {
                  ...
                  minLength: { value: 10, message: 'Je bericht mag iets uitgebreider zijn.' },
                })}
                className="w-full rounded-2xl border border-white/10 bg-background/60 px-4 py-3 text-sm text-foreground outline-none transition focus:border-highlight/70 focus:ring-2 focus:ring-highlight/60"
                placeholder="Vertel me meer over je project of vraag"
              />
              {errors.message ? <p className="text-xs text-accent">{errors.message.message}</p> : null}
            </div>
            <div className="flex items-center gap-3">
              <ShinyButton className="justify-center" icon={<span aria-hidden>→</span>} type="submit">
                {isSubmitting ? 'Versturen...' : 'Versturen'}
              </ShinyButton>
              {status === 'success' ? (
                <p className="text-xs text-highlight">Bedankt voor je bericht! Ik neem snel contact op.</p>
              ) : null}
              {status === 'error' ? (
                <p className="text-xs text-accent">Er ging iets mis. Probeer het later opnieuw of stuur een DM.</p>
              ) : null}
            </div>
          </form>
        </FadeIn>
        <FadeIn delay={0.12} className="h-full">
          <div className="flex h-full flex-col gap-6 rounded-3xl border border-white/10 bg-gradient-to-br from-white/5 via-background/70 to-transparent p-6 shadow-inner sm:p-10">
            <h3 className="font-display text-xl font-semibold text-foreground">Connecteer via socials</h3>
            <p className="text-sm text-muted-foreground">
              Liever direct een bericht sturen? Je vindt me op onderstaande platforms. Op Discord ben ik meestal het snelst bereikbaar.
            </p>
                  ...
                  >
                    <span className="flex items-center gap-3">
                      <span className="flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-background/80 text-highlight transition group-hover:border-highlight/70 group-hover:bg-highlight/10 group-hover:text-accent">
                        {socialIcons[social.label] ?? <span className="text-lg">•</span>}
                      </span>
                    ...
                    <span aria-hidden className="text-base transition group-hover:translate-x-1">
                      ↗
                    </span>
                  </a>
                </li>
